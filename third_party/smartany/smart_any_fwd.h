...
namespace detail
{
    ...
    template<bool>
    struct omaha_static_assert;

    template<>
    struct omaha_static_assert<true>
    {
        omaha_static_assert() {}
    };
...
struct close_delete
{
    ...
    static void close( T * p )
    {
        ...
        static detail::omaha_static_assert<0 != sizeof( T )> const cannot_delete_an_incomplete_type;

        #ifdef SMART_ANY_PTS
        // This checks to make sure we're not calling delete on a HANDLE
        static detail::omaha_static_assert<!detail::is_handle<T*>::value> const cannot_delete_a_handle;
        #endif
